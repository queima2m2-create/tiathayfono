// Spain (es-ES) variant — same logic, Spanish thousand separator
const BASE_DATE = new Date("2026-04-12");
const BASE_COUNT = 1873;

export function getSocialProofCountSpain(): string {
  const now = new Date();
  const days = Math.floor((now.getTime() - BASE_DATE.getTime()) / 86400000);
  if (days <= 0) return BASE_COUNT.toLocaleString("es-ES");
